...
function isCopilotPR(pr) {
    // Check if PR was created by known Copilot accounts
    ...
    const isCopilotUser = copilotUsers.has(login);
    const isCopilotBot = pr.user?.type === 'Bot' && login.includes('copilot');

    // Check PR title/body for explicit Copilot markers
    ...
    const hasLabel = pr.labels?.some(label =>
        copilotLabels.has(label.name?.toLowerCase())
    );

    return isCopilotUser || isCopilotBot || hasIndicator || hasLabel;
}
