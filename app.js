...
function isCopilotPR(pr) {
    ...
    const copilotLabels = ['copilot', 'copilot-generated', 'github-copilot'];
    const hasLabel = pr.labels && pr.labels.some(label =>
        copilotLabels.includes(label.name.toLowerCase())
    );

    const copilotSignatures = [
        'generated by github copilot',
        'created by github copilot',
        'github copilot workspace'
    ];
    const titleBody = `${pr.title} ${pr.body || ''}`.toLowerCase();
    const hasSignature = copilotSignatures.some(signature =>
        titleBody.includes(signature)
    );

    return isCopilotUser || hasLabel || hasSignature;
}
